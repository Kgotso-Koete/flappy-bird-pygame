...
def draw_floor():
    ...
    # draw second floor next to first (add width of screen)
...
def move_pipes(pipes):
    for pipe in pipes:
        pipe.centerx -= 5  # moves all pipes to the left by 5 pixels
...
def pipe_score_check():
    ...
    if pipe_list:
        for pipe in pipe_list:
            if 95 < pipe.centerx < 109 and can_score:
                score += 1
                score_sound.play()
                can_score = False
            if pipe.centerx < 0:
...
pygame.time.set_timer(SPAWNPIPE, 1200)  # trigger every 1.2 seconds
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # put background in origin in top left (x,y) coordinates
